...
<petclinic:layout pageName="runes">
	...
	<a
		...
		href="${fn:escapeXml(runeNewUrl)}">Add New Rune</a>
	<br>
	<br>
	<br>
		...
		<tbody>
			<c:forEach items="${runes}" var="rune">
				<tr>
					...
					<td><c:out value="${rune.node}" /></td>
					<td><c:out value="${rune.branch.name}" /></td>
					<td><spring:url value="/runes/{runeId}/remove"
							var="runeRemoveUrl">
							<spring:param name="runeId" value="${rune.id}" />
						</spring:url> <a href="${fn:escapeXml(runeRemoveUrl)}">Remove rune</a> <spring:url
							value="/runes/{runeId}/edit" var="editUrl">
						<spring:param name="runeId" value="${rune.id}" />
						</spring:url> <a href="${fn:escapeXml(editUrl)}" >Edit Rune</a></td>
